...
std_tokenized = tokenize_character_shingles(colnames(d_std), n = 2)
rmt_tokenized = tokenize_character_shingles(colnames(d_rmt), n = 2)


for (i in 1:ncol(d_std)) {
  
  for (j in 1:ncol(d_rmt)) {
    
    sim_mat[i,j] = jaccard(as.vector(std_tokenized[[i]]), as.vector(rmt_tokenized[[j]]))
